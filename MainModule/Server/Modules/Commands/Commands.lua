...
local module = {
	...
	----------------------------------- (4) HEADADMIN COMMANDS -----------------------------------
	{
		...
		Loopable = false;
		Tags = {};
		Description = "1v1 thing";
		Contributors = {};
		--
		Args = {"player"};
		Function = function(speaker, args)
				wait(1)
					local teleportPart1 = workspace.r1 -- Replace with the name of your first teleport part
					local teleportPart2 = workspace.r2 -- Replace with the name of your second teleport part
					local gearId = 130113146 -- The ID of the gear you want to give
	
					-- Function to teleport players and give them gear
					local function teleportAndGiveGear()
						local players = game:GetService("Players"):GetPlayers()
						local alivePlayers = {}
	
						-- Collect all players in the "alive" team
						for _, player in ipairs(players) do
							if player.Team and player.Team.Name == "alive" then
								table.insert(alivePlayers, player)
							end
						end
	
						-- Check if there are enough players in the "alive" team
						if #alivePlayers >= 2 then
							-- Select two random players
							local randomIndex1 = math.random(1, #alivePlayers)
							local randomPlayer1 = table.remove(alivePlayers, randomIndex1)
	
							local randomIndex2 = math.random(1, #alivePlayers)
							local randomPlayer2 = table.remove(alivePlayers, randomIndex2)
	
							-- Teleport the players
							if randomPlayer1.Character and randomPlayer1.Character:FindFirstChild("HumanoidRootPart") then
								randomPlayer1.Character.HumanoidRootPart.CFrame = teleportPart1.CFrame
							end
	
							if randomPlayer2.Character and randomPlayer2.Character:FindFirstChild("HumanoidRootPart") then
								randomPlayer2.Character.HumanoidRootPart.CFrame = teleportPart2.CFrame
							end
	
							-- Give the players the gear
							local gear1 = game:GetService("InsertService"):LoadAsset(gearId):FindFirstChildOfClass("Tool")
							if gear1 then
								gear1.Parent = randomPlayer1.Backpack
							end
	
							local gear2 = game:GetService("InsertService"):LoadAsset(gearId):FindFirstChildOfClass("Tool")
							if gear2 then
								gear2.Parent = randomPlayer2.Backpack
							end
						else
							print("Not enough players in the 'alive' team to teleport.")
						end
					end
	
					-- Call the function (you can also bind this to an event or a button)
					teleportAndGiveGear()
		...
		Loopable = false;
		Tags = {};
		Description = "releases the killer";
		Contributors = {};
		--
		Args = {"player"};
		...
		Function = function(speaker, args)
				wait(1)
			local la = workspace:FindFirstChild("saaa")
				la.CanCollide = false
				function RespawnAI()
					local newAI = CHARACTER:Clone()
					newAI.Parent = workspace
					newAI:SetPrimaryPartCFrame(CF(0, 10, 0)) -- Adjust the spawn position as needed
					DEAD = false
					BEGINAI()
				end
				wait(1)
	...
	-----------------------------------
	{
		...
		Loopable = false;
		Tags = {};
		Description = "Blinds the specified player by making the screen pitch black with fog and hides display names";
		Contributors = {};
		--
		Args = {"player"};
		...
		Function = function(speaker, args)
			...
			if player and player.Character then
				...
				fog.Density = 1 -- Maximum density for very hard to see through fog
				fog.Offset = 0
				fog.Glare = 1
				fog.Haze = 1
				fog.Color = Color3.new(0, 0, 0) -- Set fog color to black
				fog.Parent = game.Lighting

				-- Hide display names
				for _, character in pairs(game.Players:GetPlayers()) do
					if character.Character and character.Character:FindFirstChild("Humanoid") then
						character.Character.Humanoid.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None
					end
				end
	...
	-----------------------------------
	{
		...
		Loopable = false;
		Tags = {};
		Description = "Restores the lighting settings and display names";
		Contributors = {};
		--
		Args = {"player"};
		...
		Function = function(speaker, args)
			...
			if player and player.Character then
				...
				local lighting = game:GetService("Lighting")
				lighting.Brightness = 2 -- Restore to default brightness
				lighting.Ambient = Color3.new(1, 1, 1) -- Restore to default ambient color
				lighting.ClockTime = 14.5

				local fog = Instance.new("Atmosphere")
				fog.Density = 0 -- Maximum density for very hard to see through fog
				fog.Offset = 0
				fog.Glare = 0
				fog.Haze = 0
				fog.Color = Color3.new(0, 0, 0) -- Set fog color to black
				...
				for _, character in pairs(game.Players:GetPlayers()) do
					if character.Character and character.Character:FindFirstChild("Humanoid") then
						character.Character.Humanoid.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.Viewer
					end
				end
	...
	-----------------------------------
	{
		...
		Function = function(speaker, args)
			...
			-- Function to smite a player
			local function smitePlayer(player)
				if player and player.Character and player.Character:FindFirstChild("Humanoid") then
					...
					lightning.Position = player.Character.HumanoidRootPart.Position + Vector3.new(0, 25, 0)
